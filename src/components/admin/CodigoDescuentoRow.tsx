...
export default function CodigoDescuentoRow({
  ...
  return (
    <div className="grid grid-cols-[1fr_1fr_1fr_1fr_1fr_6rem] items-center gap-4 border-t border-black/8 px-4 py-4 text-sm dark:border-white/[.145]">
      <span className="font-mono font-semibold">{codigo.codigo}</span>
      <span>
        {codigo.tipo === "porcentaje" ? `${codigo.valor}%` : formatoPrecio.format(codigo.valor)}
      ...
      <span className="text-zinc-600 dark:text-zinc-400">
        {codigo.vigente_hasta ? formatoFecha(codigo.vigente_hasta) : "Sin vencimiento"}
      </span>
      <span className="text-zinc-600 dark:text-zinc-400">
